  ...
  <body class="min-h-screen flex flex-col selection:bg-secondary selection:text-black">
    ...
    <header class="border-b border-secondary/15 bg-black/80 sticky top-0 z-40 backdrop-blur-md">
      <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
          <a href="admin.php" class="text-xs font-semibold bg-white/5 border border-white/10 hover:bg-white/10 text-white px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1">
    ...
    <main class="flex-1 max-w-6xl w-full mx-auto p-4 grid md:grid-cols-12 gap-6 items-start">
      ...
      <section class="md:col-span-7 flex flex-col gap-6">
        ...
        <!-- Live Scanner -->
        <div class="bg-[#141414] border border-secondary/15 rounded-3xl p-6 shadow-2xl">
          <div class="flex items-center justify-between mb-4 border-b border-secondary/10 pb-3">
            <div class="flex items-center gap-2">
              ...
              <h2 class="font-display font-semibold text-lg text-white">Live Camera QR Scanner</h2>
            </div>
          ...
          <div class="relative w-full aspect-video md:aspect-[4/3] bg-black rounded-2xl overflow-hidden border border-white/5 flex flex-col items-center justify-center">
            <div id="reader" class="w-full h-full"></div>
            <div id="scanner-placeholder" class="absolute inset-0 flex flex-col items-center justify-center bg-[#070708] p-4 text-center">
              ...
              <p class="text-xs text-gray-500 mt-1 max-w-xs">Klik tombol "Mulai Kamera" di atas untuk menggunakan kamera device ini sebagai scanner ticket.</p>
            </div>
          </div>
          ...
          <div class="mt-5 pt-4 border-t border-white/5 flex gap-3">
            <input type="text" id="manual-code" placeholder="Input kode manual (e.g. GCP-A1B2C)" 
              class="flex-1 bg-[#0C0C0C] border border-secondary/20 rounded-xl px-4 py-2 text-white text-sm focus:border-secondary focus:ring-1 focus:ring-secondary/50 placeholder-gray-600 uppercase" />
        ...
        <div class="bg-[#141414] border border-secondary/10 rounded-2xl p-4">
      ...
      <section class="md:col-span-5 flex flex-col gap-6">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          ?>
          <div class="bg-[#141414] border border-secondary/10 rounded-2xl p-4 text-center shadow-lg">
            <p class="text-xxs uppercase tracking-widest text-gray-500 mb-1">Total Hadir</p>
            <p class="text-2xl font-bold font-display text-secondary" id="stat-present"><?= $totalPresent ?></p>
          </div>
          <div class="bg-[#141414] border border-secondary/10 rounded-2xl p-4 text-center shadow-lg">
            <p class="text-xxs uppercase tracking-widest text-gray-500 mb-1">Total Undangan</p>
            <p class="text-2xl font-bold font-display text-white" id="stat-total"><?= $totalGuests ?></p>
          </div>
        </div>

        <!-- Recent Checkins -->
        <div class="bg-[#141414] border border-secondary/15 rounded-3xl p-5 shadow-2xl">
          <div class="flex items-center justify-between mb-4 border-b border-secondary/10 pb-3">
          ...
          <div class="max-h-[360px] overflow-y-auto custom-scrollbar pr-1 flex flex-col gap-3" id="checkin-list">
            ...
            <?php else: ?>
              <?php foreach ($recentCheckins as $check): ?>
                ...
                ?>
                <div class="bg-[#0C0C0C] border border-white/5 hover:border-secondary/10 rounded-xl p-3.5 flex justify-between items-center transition-all duration-300">
                  <div class="flex-1 min-w-0 pr-3">
                    <h4 class="text-white text-xs font-bold truncate uppercase"><?= htmlspecialchars($check['name']) ?></h4>
                    <p class="text-[10px] text-gray-400 mt-0.5"><?= htmlspecialchars($check['classroom']) ?> • <span class="text-secondary/70"><?= $comp ?></span></p>
                  </div>
                  <div class="text-right flex flex-col items-end">
                    <span class="text-[10px] text-gray-500 font-medium"><?= date('H:i:s', strtotime($check['checked_in_at'])) ?></span>
    ...
    <div id="result-modal" class="fixed inset-0 bg-black/95 z-50 hidden items-center justify-center p-5 backdrop-blur-md">
      <div id="result-card" class="bg-[#141414] border-2 max-w-sm w-full p-8 rounded-3xl text-center shadow-2xl transition-all duration-500 transform scale-95 opacity-0">
        <span id="result-icon" class="material-symbols-outlined text-6xl mb-4">check_circle</span>
        
        <h3 id="result-title" class="font-display font-bold text-2xl text-white mb-1">CHECK-IN BERHASIL</h3>
        <p id="result-subtitle" class="text-xs text-gray-400 uppercase tracking-widest mb-6">Informasi Kehadiran</p>
        
        <!-- Details box -->
        <div class="bg-black/40 border border-white/5 rounded-2xl p-4 text-left mb-6 space-y-2">
          <div>
            <span class="text-[9px] text-gray-500 uppercase tracking-wider">Nama Tamu</span>
            <p id="res-name" class="text-white font-bold text-sm uppercase break-words">-</p>
          </div>
          <div class="grid grid-cols-2 gap-2">
            <div>
              <span class="text-[9px] text-gray-500 uppercase tracking-wider">Kelas</span>
              <p id="res-class" class="text-white font-bold text-xs uppercase">-</p>
            </div>
            <div>
              <span class="text-[9px] text-gray-500 uppercase tracking-wider">Pendamping</span>
              <p id="res-companion" class="text-secondary font-bold text-xs uppercase">-</p>
            </div>
          </div>
          <div>
            <span class="text-[9px] text-gray-500 uppercase tracking-wider">Waktu Masuk</span>
            <p id="res-time" class="text-white text-xs font-semibold">-</p>
          </div>
        </div>
