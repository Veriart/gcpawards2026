...
<html lang="en">
  <head>
    ...
    <style>
      body { background-color: #050505; color: #f3f4f6; font-family: 'Poppins', sans-serif; }
    </style>
  </head>
  <body class="min-h-screen flex items-center justify-center p-5 selection:bg-[#D4AF37] selection:text-black">
    <div class="w-full max-w-md bg-[#141414] border border-[#D4AF37]/20 rounded-3xl p-8 sm:p-10 shadow-2xl relative overflow-hidden">
      
      <!-- Ornamental border -->
      <div class="absolute inset-2 border border-[#D4AF37]/5 rounded-2xl pointer-events-none"></div>

      <div class="text-center mb-8 relative z-10">
        <img src="img/metschoo/Metschoo.png" alt="Logo" class="w-14 h-14 object-contain mx-auto mb-4" />
        <h1 class="font-display font-bold text-white text-xl uppercase tracking-wider">GCP Award 2026</h1>
        <p class="text-xs text-[#D4AF37] tracking-widest uppercase mt-1">Admin Dashboard Portal</p>
      </div>
      ...
      <form action="admin.php" method="POST" class="space-y-5 relative z-10">
        <div>
          <label class="block text-gray-400 text-xs font-semibold uppercase tracking-wider mb-2 px-1">Username</label>
          <input type="text" name="username" required placeholder="Enter username" 
            class="w-full bg-[#0C0C0C] border border-[#D4AF37]/20 rounded-2xl px-4 py-3.5 text-white text-sm focus:border-[#D4AF37] focus:ring-1 focus:ring-[#D4AF37]/40 transition-all outline-none" />
        </div>
        
        <div>
          <label class="block text-gray-400 text-xs font-semibold uppercase tracking-wider mb-2 px-1">Password</label>
          <input type="password" name="password" required placeholder="Enter password" 
            class="w-full bg-[#0C0C0C] border border-[#D4AF37]/20 rounded-2xl px-4 py-3.5 text-white text-sm focus:border-[#D4AF37] focus:ring-1 focus:ring-[#D4AF37]/40 transition-all outline-none" />
        </div>

        <button type="submit" name="login" class="w-full bg-[#D4AF37] text-black font-bold py-4 rounded-2xl text-xs tracking-widest uppercase hover:bg-[#D4AF37]/90 active:scale-95 transition-all shadow-lg mt-2">
          LOG IN ADMIN
        </button>
      </form>
    </div>
...
<html lang="en">
  ...
  <body class="min-h-screen flex flex-col selection:bg-secondary selection:text-black">
    
    <!-- Top Nav -->
    <header class="border-b border-secondary/15 bg-black/80 sticky top-0 z-40 backdrop-blur-md">
      <div class="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        <div class="flex items-center gap-2">
          <img src="img/metschoo/Metschoo.png" alt="Logo" class="w-8 h-8 object-contain" />
          <div>
            <h1 class="font-display font-bold text-white text-base tracking-wide leading-none">GCP Award 2026</h1>
    ...
    <main class="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <section class="grid grid-cols-2 md:grid-cols-5 gap-4">
        <!-- Card 1: Total -->
        <div class="bg-[#141414] border border-secondary/10 rounded-2xl p-4 shadow-lg text-center">
          <p class="text-[10px] uppercase tracking-wider text-gray-500 font-medium">Total Undangan</p>
          <p class="text-3xl font-display font-bold text-white mt-1"><?= $totalCount ?></p>
        </div>
        <!-- Card 2: Attending -->
        <div class="bg-[#141414] border border-[#25D366]/20 rounded-2xl p-4 shadow-lg text-center">
          <p class="text-[10px] uppercase tracking-wider text-gray-500 font-medium">Hadir (RSVP)</p>
          <p class="text-3xl font-display font-bold text-[#25D366] mt-1"><?= $attendingCount ?></p>
        </div>
        <!-- Card 3: Absent -->
        <div class="bg-[#141414] border border-red-600/20 rounded-2xl p-4 shadow-lg text-center">
          <p class="text-[10px] uppercase tracking-wider text-gray-500 font-medium">Absen (RSVP)</p>
          <p class="text-3xl font-display font-bold text-red-500 mt-1"><?= $absentCount ?></p>
        </div>
        <!-- Card 4: Pending -->
        <div class="bg-[#141414] border border-yellow-600/20 rounded-2xl p-4 shadow-lg text-center">
          <p class="text-[10px] uppercase tracking-wider text-gray-500 font-medium">Pending (RSVP)</p>
          <p class="text-3xl font-display font-bold text-yellow-500 mt-1"><?= $pendingCount ?></p>
        </div>
        <!-- Card 5: Present Checked In -->
        <div class="bg-[#141414] border border-secondary/25 rounded-2xl p-4 shadow-lg text-center col-span-2 md:col-span-1">
          ...
          <!-- Companion Substats -->
          <div class="text-[8px] text-gray-500 mt-1 flex justify-center gap-2">
      ...
      <section class="grid md:grid-cols-2 gap-6">
        ...
        <div class="bg-[#141414] border border-secondary/10 rounded-3xl p-5 shadow-xl">
          <h3 class="font-display font-semibold text-white mb-4 flex items-center gap-2 border-b border-white/5 pb-2">
          ...
          <form action="admin.php" method="POST" class="grid grid-cols-2 gap-4">
            <div class="col-span-2">
              <label class="block text-gray-400 text-xxs uppercase tracking-wider mb-1 px-1">Nama Lengkap</label>
              <input type="text" name="name" required placeholder="Nama Siswa"
                class="w-full bg-[#0C0C0C] border border-secondary/20 rounded-xl px-3.5 py-2 text-white text-xs focus:border-secondary focus:ring-1 focus:ring-secondary/40 outline-none" />
            </div>
            <div>
              <label class="block text-gray-400 text-xxs uppercase tracking-wider mb-1 px-1">Kelas / Afiliasi</label>
        ...
        <div class="bg-[#141414] border border-secondary/10 rounded-3xl p-5 shadow-xl">
          ...
          <form action="admin.php" method="POST" class="flex flex-col gap-3">
            <div>
              <label class="block text-gray-400 text-xxs uppercase tracking-wider mb-1 px-1">Teks Data (Format: Nama [Tab atau Koma] Kelas)</label>
              <textarea name="bulk_text" required placeholder="Budi Santoso&#9;XII PPLG 1&#10;Ani Herawati&#9;XII AKT" rows="2"
                class="w-full bg-[#0C0C0C] border border-secondary/20 rounded-xl px-3 py-2 text-white text-xs focus:border-secondary focus:ring-1 focus:ring-secondary/40 outline-none resize-none custom-scrollbar"></textarea>
            </div>
            <div class="flex justify-between items-center gap-3">
              <div class="flex-1 flex items-center gap-2">
                <span class="text-gray-400 text-xxs uppercase tracking-wider whitespace-nowrap">Default Kelas:</span>
                <input type="text" name="default_classroom" value="Guest" 
                  class="w-full bg-[#0C0C0C] border border-secondary/20 rounded-xl px-2 py-1 text-white text-xs focus:border-secondary focus:ring-1 focus:ring-secondary/40 outline-none" />
              </div>
      ...
      <section class="bg-[#141414] border border-secondary/15 rounded-3xl p-5 shadow-2xl space-y-4">
        
        <!-- Search and filters -->
        <form method="GET" action="admin.php" class="grid grid-cols-2 md:grid-cols-5 gap-3 items-end">
          <div class="col-span-2 md:col-span-2">
            <label class="block text-gray-400 text-xxs uppercase tracking-wider mb-1 px-1">Cari Nama / Kode</label>
            <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Masukkan kata kunci..."
              class="w-full bg-[#0C0C0C] border border-secondary/20 rounded-xl px-3 py-2 text-white text-xs focus:border-secondary outline-none" />
          </div>
          <div>
            <label class="block text-gray-400 text-xxs uppercase tracking-wider mb-1 px-1">Filter Kelas</label>
            <select name="filter_class" class="w-full bg-[#0C0C0C] border border-secondary/20 rounded-xl px-2 py-2 text-white text-xs focus:border-secondary outline-none appearance-none">
          ...
          <div>
            <label class="block text-gray-400 text-xxs uppercase tracking-wider mb-1 px-1">Filter RSVP</label>
            <select name="filter_rsvp" class="w-full bg-[#0C0C0C] border border-secondary/20 rounded-xl px-2 py-2 text-white text-xs focus:border-secondary outline-none appearance-none">
          ...
          <div class="flex gap-2">
            <div class="flex-1">
              <label class="block text-gray-400 text-xxs uppercase tracking-wider mb-1 px-1">Filter Kehadiran</label>
              <select name="filter_checkin" class="w-full bg-[#0C0C0C] border border-secondary/20 rounded-xl px-2 py-2 text-white text-xs focus:border-secondary outline-none appearance-none">
        ...
        <!-- Main table -->
        <div class="overflow-x-auto custom-scrollbar border border-white/5 rounded-2xl">
          <table class="w-full text-left border-collapse min-w-[900px]">
            <thead>
              <tr class="bg-black text-[10px] text-gray-400 font-semibold uppercase tracking-wider border-b border-secondary/15">
              ...
              <?php else: ?>
                <?php foreach ($students as $row): ?>
                  ...
                  <tr class="hover:bg-white/5 transition-colors duration-200">
                    ...
                    <!-- Name -->
                    <td class="py-3.5 px-4 font-medium text-white uppercase"><?= htmlspecialchars($row['name']) ?></td>
                    <!-- Classroom -->
                    <td class="py-3.5 px-4 text-gray-300"><?= htmlspecialchars($row['classroom']) ?></td>
                    <!-- Whatsapp -->
                    <td class="py-3.5 px-4 text-gray-400 font-mono text-xxs truncate max-w-[120px]" title="<?= htmlspecialchars($row['whatsapp'] ?? '') ?>">
                    ...
                    <td class="py-3.5 px-4">
                      <?php if ($row['companion_type'] === 'parents'): ?>
                        <span class="text-white text-[10px] bg-white/5 border border-white/10 px-2 py-0.5 rounded">Father/Mother</span>
                      <?php elseif ($row['companion_type'] === 'sibling'): ?>
                        <span class="text-white text-[10px] bg-white/5 border border-white/10 px-2 py-0.5 rounded">Saudara</span>
                      <?php else: ?>
                        <span class="text-gray-600 text-[10px] italic">Sendiri</span>
                      <?php endif; ?>
                    </td>
                    <!-- Check-in status -->
                    <td class="py-3.5 px-4">
                      <?php if ($row['checked_in'] == 1): ?>
                        <div class="flex flex-col items-start">
                          ...
                          <span class="text-[8px] text-gray-500 mt-0.5"><?= date('H:i:s', strtotime($row['checked_in_at'])) ?></span>
                        </div>
                      <?php else: ?>
                        <span class="text-gray-600 font-medium italic">Belum</span>
                      <?php endif; ?>
                    </td>
                    <!-- Actions -->
                    <td class="py-3.5 px-4 text-center space-x-1 whitespace-nowrap">
                      ...
                      <!-- Copy invite Link -->
                      <button onclick="copyToClipboard('<?= $inviteLink ?>', this)" class="inline-flex items-center justify-center w-7 h-7 bg-white/5 text-white hover:bg-white/10 border border-white/10 rounded-lg transition-all" title="Copy Link Undangan">
    ...
    <footer class="bg-black py-6 border-t border-secondary/15 text-center text-[10px] tracking-widest text-gray-500 uppercase">
